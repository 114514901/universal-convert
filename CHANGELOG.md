...
## [2.2.0-dev.9] - 2026-08-25

### 改进
- 下载资源调度升级：全局连接预算 9（由块级并发控制，不再每文件固定 8 段）；文件切成 8MB 块竞争预算——单文件独占 9 连接、多文件按进度动态分配，**文件完成自动把连接让给剩余文件**（最后剩下的文件能拿满）；扩展并行下载上限提到 3 个文件

## [2.2.0-dev.8] - 2026-08-25

### 新增
- 扩展安装/更新支持**暂停/继续与取消**：下载中可暂停（挂起分段下载，恢复自动续传；暂停过久导致连接被掐断的段在恢复后自动重下）、取消（终止所有任务，行状态显示「已取消」，全部取消不弹重启提示）

## [2.2.0-dev.7] - 2026-08-25

### 新增
- 文本文件预览器：双击/预览常见文本（txt/md/log/json/xml/csv/ini/srt/lrc/yaml 等）内置只读查看；**JSON 语法高亮**（键/字符串/数字/布尔/标点着色，容错非严格 JSON）；UTF-8 解码失败自动回退 GBK；超大文件（≥1MB）跳过高亮保证流畅

## [2.2.0-dev.6] - 2026-08-25

### 修复
- 旧版扩展（LibreOffice/MIDI 等）升级主程序后报「找不到方法」：ProcessRunner.Run 增加可选参数改变了方法签名（可选参数是编译期糖），旧扩展二进制绑定的 3 参/5 参签名在新 Core 中不存在——补物理重载向后兼容，旧扩展无需重编译即可运行
- 错误解析器改进：识别「找不到方法/类型」等版本不兼容错误，给出「更新扩展到最新版本」的明确建议（此前被误归类为「未找到所需工具」）
...
### 修复
- 语音转文字等 PyInstaller 单文件工具的取消/暂停无效：工具是"外壳进程 + 实际工作子进程"两级结构，只杀/挂起外壳会留孤儿进程继续运行——取消改为 `taskkill /T` 杀整棵进程树，暂停/恢复改为作用于全部子进程
- 带封面的音频（如 mp3 内嵌专辑封面图）转 m4a 等音频格式失败：封面被当视频流转码进不支持的容器——现在 mp3/m4a/flac 目标保留封面（mjpeg 原样 copy），ogg/opus/aac 等丢弃封面但转换成功

## [2.2.0-dev.3] - 2026-08-25

### 修复
- 右键多选批量转换点击后错误打开主界面：convert 模式的多文件在 ExtraFiles 中（InputPath 为空），启动分支误判——现在多选文件会正确弹出批量转换窗口并立即开始

## [2.2.0-dev.2] - 2026-08-25

### 修复
- 音频预览默认音量与实际不符：音量条初始 100% 但 MediaPlayer 默认音量只有 50%——现在默认满音量，拉一下不会"突然变大"
- 更新安装时 iTop Easy Desktop 锁定插件 DLL 导致覆盖失败（拒绝访问）：安装器在复制文件前自动结束 iTop 进程（桌面整理类软件会枚举右键菜单连带加载插件 DLL）

## [2.2.0-dev.1] - 2026-08-25

### 新增
- 右键菜单支持**同格式多选批量转换**：资源管理器多选相同扩展名的文件，右键「转换为」直接批量转换（弹出批量转换窗口，含进度/暂停/取消，支持预设）；混合格式多选保持仅「打开」
- NCM/KGM/QMC 加密音乐**临时解密预览**：文件列表双击加密音乐直接试听（解密到临时文件播放，关闭预览自动清理）
