...
## [2.3.0-dev.7] - 2026-08-28

### 修复
- 视频预览打开即崩溃（NullReferenceException）：XAML 加载期间音量滑块 Value 初始化先于 VolumeText 字段赋值，事件处理访问空引用——判空修复；音频预览音量处理同款隐患一并防御

## [2.3.0-dev.6] - 2026-08-27

### 新增
- **视频预览**：文件列表双击/右键视频（mp4/avi/wmv/mkv/webm 等）内置播放器直接预览（播放/暂停/停止/进度拖动/音量，MediaElement）；系统解码器不支持的格式自动回落系统默认播放器

## [2.3.0-dev.5] - 2026-08-27

### 改进
- 更新横幅自动同步：设置里手动检查发现比横幅更新的版本时，关闭设置后主界面横幅自动刷新为新版本（含版本号/下载/查看内容）

## [2.3.0-dev.4] - 2026-08-27

### 改进
- 音频预览音量改为**指数曲线**（人耳对数感知，Windows 风格）：滑块低段敏感、高段平滑，100% = 满音量；新增音量百分比显示

## [2.3.0-dev.3] - 2026-08-27

### 新增
- 插件管理器支持**多选批量卸载**（与扩展中心一致）：选中多个用户扩展一次确认卸载，逐项更新状态，部分失败有提示、有暂存项提示重启

## [2.3.0-dev.2] - 2026-08-25

### 修复
- 图片预览：适应窗口后滚轮缩放从实际显示比例起步；缩放改用 Width/Height 控制显示尺寸（ScaleTransform 只影响渲染导致滚动条/拖拽失效）——缩放保持视口中心、滚动与拖拽平移正常
- JSON 高亮配色区分并升级为 **VSCode 式括号配对着色**：配对的括号同色、嵌套层级循环换色（金/紫/青/红），字符串绿色、键蓝色；字体改复合字体（Consolas + 雅黑）并加大

### 新增
- 文本预览支持 **YAML/yml 高亮**（键/字符串/数字/布尔/注释/列表符）
- 扩展管理器支持**多选批量卸载**（一次确认卸载选中项，暂存项提示重启）
- 扩展下载进度显示 **已下载/总大小 MB**（保留一位小数，如 3.4/512.0 MB）

## [2.2.0] - 2026-08-25

### 修复
- 图片预览：适应窗口后滚轮缩放从实际显示比例起步（不再是 125%/80%）；缩放改用 Width/Height 控制显示尺寸（ScaleTransform 只影响渲染导致滚动条/拖拽失效）——缩放后图片保持视口中心、滚动与拖拽平移正常
- JSON 高亮配色区分：花括号（金）与字符串值（绿）不再同色；`{}` 金、`[]` 紫、字符串绿、键蓝、数字橙、布尔蓝；字体改复合字体（Consolas + 雅黑）并加大

### 新增
- 文本预览支持 **YAML/yml 高亮**（键蓝、字符串绿、数字橙、布尔蓝、注释灰绿、列表符灰）
- 扩展管理器支持**多选批量卸载**（一次确认卸载选中项，暂存项提示重启）
- 扩展下载进度显示 **已下载/总大小 MB**（保留一位小数，如 3.4/512.0 MB）
