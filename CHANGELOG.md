...
## [2.0.2-dev.9] - 2026-08-25

### 修复
- 检查扩展更新后弹窗选「否」，状态栏仍停留「正在检查扩展更新…」：改为提示「已跳过更新」，单插件检查同样处理

## [2.0.2-dev.8] - 2026-08-25
...
### 改进
- 扩展检查更新：只检查用户扩展、并行比对；发现更新后弹窗确认是否立即更新
- 扩展更新改为「暂存 + 重启后生效」（已加载插件 DLL 被进程锁定无法直接替换），启动时自动应用
