...
## [2.3.0-dev.9] - 2026-08-28

### 修复与改进
- 视频预览：**停止后无法再播放**（WPF MediaElement Stop 后直接 Play 无效的已知问题）——播放前重新赋值 Source
- 进度条**拖动时实时预览该位置画面**（拖动中实时 seek 渲染对应帧，松手定位）

## [2.3.0-dev.8] - 2026-08-28

### 新增
- 崩溃报告窗口加「**继续运行**」按钮（UI 线程崩溃后可带病继续，不再只能重启/关闭）；原「关闭」改为「**结束进程**」（走正常退出流程）；窗口顶部新增风险提示：继续运行可能导致状态异常、崩溃可能重现
