...
### 新增
- 转换支持**暂停/继续与取消**：批量转换窗口新增「暂停/继续」「取消」按钮，单文件转换窗口新增「暂停/继续」（取消已有）
- 暂停通过挂起外部工具进程实现（批量时所有运行中任务一起挂起、未开始的任务在引擎入口等待）；取消立即终止当前进程、跳过剩余任务
- 所有继承 `ExternalToolConverterBase` 的插件（内置 FFmpeg、Pandoc/MarkItDown/ImageMagick 扩展）自动获得暂停能力；转换请求新增 `PauseSignal`（Core API）

## [2.1.0-dev.7] - 2026-08-25

### 新增
- 下载完整性校验（SHA256）：软件更新从 GitHub 资产 digest 动态获取并校验安装包；扩展安装/更新从 index.json 的 sha256 字段校验扩展包，校验失败中止并提示，防止下载损坏或被篡改

## [2.1.0-dev.6] - 2026-08-25

### 新增
- 扩展中心列表新增「体积」列：扩展仓库 index.json 的 size 字段（字节）自动格式化为 MB/GB，装大扩展（如 LibreOffice ~400MB）前用户可先看体积

## [2.1.0-dev.5] - 2026-08-25

### 修复
- 外部工具转换在只读工作目录下失败（如从 Program Files 启动时，ffmpeg/markitdown 无法在当前目录创建临时文件，报「没有写入权限」）：进程工作目录默认改为系统临时目录

## [2.1.0-dev.4] - 2026-08-25

### 改进
- 看护进程正常退出机制：主程序退出前发送退出信号（命名事件），看护进程立即退出，不再靠轮询发现（此前最多延迟 3 秒）；卡死检测逻辑不变

## [2.1.0-dev.3] - 2026-08-25

### 改进
- 扩展安装/更新全链路诊断日志：暂存是否触发及原因、暂存 PID、启动时应用结果、失败原因逐条记录到 app.log，便于定位「重启后未生效」类问题
