...
## [2.4.0-dev.6] - 2026-08-28

### 改进
- 视频预览拖动进度条改**ffmpeg 直接抽帧**显示预览（MediaElement 暂停态渲染不可靠）——拖动位置画面抽成 PNG 浮层显示（150ms 节流，过期请求丢弃，松手隐藏），绕过 WPF 限制

## [2.4.0-dev.5] - 2026-08-28

### 修复
- 视频预览拖动进度条仍无预览帧：同步 Play/Pause 无效（Play 异步启动，Pause 在真正渲染前执行）——改为**静音播放 50ms** 让 MediaElement 实际渲染 seek 帧后再暂停恢复音量（有预览且无声音）

## [2.4.0-dev.4] - 2026-08-28

### 安全修复
- 工具解析**不再回退系统 PATH**（转换主流程与音频/视频/图片预览兜底均是）：PATH 中的同名工具可能是旧版/带漏洞版本（如 ffmpeg < 8.1.2 的 CVE-2026-8461 越界写入可执行任意代码）——现在只使用随包/扩展自带工具与用户显式配置的路径，缺失时明确提示安装

## [2.4.0-dev.3] - 2026-08-28

### 改进
- CI 打包时打印 ffmpeg 版本到构建日志（master 滚动快照，每个 release 的 ffmpeg 版本可追溯，便于核对安全修复状态）

## [2.4.0-dev.2] - 2026-08-28

### 修复
- 视频预览拖动进度条时画面不更新（暂停态 MediaElement 不渲染 seek 帧）：seek 后短暂 Play/Pause 强制渲染当前帧（40ms 节流，无声音）

## [2.4.0-dev.1] - 2026-08-28

### 修复
- 音频/视频预览**拖拽进度条期间临时暂停**（反复 seek 会产生噪声/杂音），松手恢复原播放状态；拖动中仍实时预览画面帧
