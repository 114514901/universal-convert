...
## [2.4.1-dev.6] - 2026-08-29

### 改进
- 加密/渲染类音频预览：渲染期间显示等待提示（如 MIDI 合成不再无反馈）；渲染产物改名为「原文件名+新扩展名」，播放器标题可读（不再显示乱码临时名）

## [2.4.1-dev.5] - 2026-08-29

### 改进
- 加密/渲染类音频（ncm/kgm/qmc/midi）**先渲染/解密成临时文件，再交给播放器（VLC 扩展优先，回退内置）**——NCM 也能用上 VLC 播放器体验；渲染临时文件 20 分钟后延迟清理

## [2.4.1-dev.4] - 2026-08-29

### 修复
- 加密/渲染类音频（ncm/kgm/qmc/midi，有 IPreviewProvider 的）不再被 VLC 等媒体提供者接管：优先走内置播放器渲染/解密再播，普通音频才轮到 VLC

## [2.4.1-dev.3] - 2026-08-29

### 新增
- `IMediaPreviewProvider` 接口：媒体预览提供者可同时接管音频与视频预览（音频分支优先调用，回退内置播）；VLC 扩展将用其接管音频，避免音频预览兜底落到系统播放器

## [2.4.1-dev.2] - 2026-08-29

### 修复
- 看护进程卡死报告窗口**禁用「继续运行」**（进程即将结束，继续无意义且引发连锁崩溃）；「继续运行」不再设置 DialogResult（兼容非 ShowDialog 场景，杜绝设置时报错）

## [2.4.1-dev.1] - 2026-08-29

### 改进
- 视频预览提供者接管路径诊断日志：接管/拒绝/异常均记录（附完整堆栈），排查 VLC 扩展未生效类问题

## [2.4.0-dev.9] - 2026-08-28

### 新增
- 插件预览接口 `IVideoPreviewProvider`：扩展可声明以自有播放器接管视频预览（如内嵌 VLC 的扩展，全格式播放 + 原生 seek 预览）；主程序预览视频时优先调用，无扩展/拒绝时回退内置预览；纯新增接口，旧扩展不受影响

## [2.4.0-dev.8] - 2026-08-28

### 新增
- 设置「常规」新增**预览播放器**选项（默认内置）：内置 = 现有预览流程（音频/图片/视频/文本 + 兜底）；系统 = 直接调用系统默认程序打开

## [2.4.0-dev.7] - 2026-08-28

### 改进
- 视频预览解码失败**ffmpeg 转码兜底**：系统解码器不支持的格式自动转成 H.264+AAC mp4 临时文件播放（标题显示转码进度），转码失败才回落系统默认播放器；关闭窗口终止转码并清理临时文件
